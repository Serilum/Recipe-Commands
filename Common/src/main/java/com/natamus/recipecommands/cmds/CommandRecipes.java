...
public class CommandRecipes {
	...
	private static void sendRecipe(CommandContext<CommandSourceStack> command) throws CommandSyntaxException, NoSuchMethodError {
		...
		for (Ingredient ingredient : ingredients) {
			List<Holder<Item>> possiblestacks = ingredient.items().toList();
			...
			if (possiblestacks.size() > 1 && !itemname.equalsIgnoreCase("cobblestone")) {
				Set<TagKey<Item>> tags = itemstack.tags().collect(Collectors.toSet());
				if (!tags.isEmpty()) {
		...
		MessageFunctions.sendTranslatableMessage(source, "collective.recipecommands.message.recipe", true, ChatFormatting.DARK_GREEN, formattedOutputName, shape);
